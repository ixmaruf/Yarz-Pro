...
(function() {
  ...
  // âœ… FIX #23: Smart db getter. Tries to use SERVICE ROLE key if available
  ...
  async function saveOrderFromForm(p) {
    var db = getWriteDb();
    await ensureAuth();
    // âœ… FIX #29: Accept both short (GAS) and long (Supabase) field names
  ...
  // ============================================================
  // CUSTOMER MANAGEMENT (separate from product delete)
  // ============================================================
  ...
  async function handleAppsPost(action, payload) {
    ...
    try {
      switch (act) {
        ...
        case "deleteSingleImage":
          return ok({ msg: "Image deletion handled by worker" });
        case "getcustomerslist":
        case "getCustomersList":
          return await sheetRead({ range: "WEBSITE_ORDERS" });
        case "diagnoses3xl":
        case "diagnoseS3XL":
        case "diagnose3xl":
          return ok({ msg: "S/3XL columns present" });
        case "githubsyncnow":
        case "githubSyncNow":
          return ok({ msg: "GitHub sync runs on worker" });
        case "publishtocloudflare":
        case "publish_to_cloudflare":
        case "publishToCloudflare":
          return ok({ msg: "Cloudflare publish runs on worker" });
        case "__currentmonthsnapshot":
        case "__currentMonthSnapshot":
          return null;
        default:
          return null;
