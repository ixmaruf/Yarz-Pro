...
const searchReg = /return \{ key, label:EN_DAYS\[target\.getDay\(\)\], isToday:false, date:target, dayName:EN_DAYS\[target\.getDay\(\)\] \};[\s\S]*?throw new Error\(res\.msg \|\| 'Sheet read failed'\);\r?\n\}/;
...
if(searchReg.test(html)) {
  html = html.replace(searchReg, newStr);
  fs.writeFileSync('Yarz-admin panal/index.html', html);
